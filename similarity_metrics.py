	...
	for m,f in zip(methods,outfiles):
		...
		for u in examples:
			for v in examples[u]:
				if (int(u) in nodes and int(v) in nodes):
					if (m=='common_neighbors'):
						b_sim[u][v] = common_neighbors(hop2s[int(u)], neighbors[int(v)])
					elif (m=='jaccard'):
						b_sim[u][v] = jaccard(hop2s[int(u)], neighbors[int(v)])
					elif (m=='adamic_adar'):
						b_sim[u][v] = adamic_adar(hop2s[int(u)], neighbors[int(v)],G)
				else:
					b_sim[u][v]=0
